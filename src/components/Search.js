    ...
    return (
      <div>
        <Div css={{
          ...
          <CloseSearch to="/"><ArrowBack css={{ width: 28, height: 28, fill: 'black' }} /></CloseSearch>
        ...
        <Div css={{ padding: '80px 10px 20px' }}>
          <BooksGrid>
            {results.length === 0 ? <Div>No search results found</Div> : map(props =>
              <BookView key={props.title} {...props}
                        updateBooks={updateBooks} />, results)}
          </BooksGrid>
        </Div>
      </div>
    )
